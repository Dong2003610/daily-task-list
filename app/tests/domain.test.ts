...
import {
  carryCandidates, carryId, carryRow, dateFromDay, dayKey, dayLabel, dueReminders,
  exportCsv, isRuleDue, reminderKey, repeatId, repeatRow, RULE_PREFIX,
  rulesFromMetadata, validInput, yesterday, cancelledSources, cancelledTaskIds,
  CARRY_CANCEL, SKIP_PREFIX,
} from '../src/domain';
import {NOW, rule, task, TODAY, USER_ID} from './factories';

describe('Shanghai calendar boundaries', () => {
  it.each([
    ['2026-01-01', '2025-12-31'], ['2026-05-01', '2026-04-30'],
    ['2026-03-01', '2026-02-28'], ['2024-03-01', '2024-02-29'],
    ['2000-03-01', '2000-02-29'], ['2100-03-01', '2100-02-28'],
...
describe('cancellation metadata compatibility', () => {
  it('suppresses legacy and current carry sources, hiding only explicitly identified copies', () => {
    const source = task({id: 'cancelled-source', created_at: '2026-09-07T01:00:00Z'});
    ...
    expect([...cancelledSources(metadata)].sort()).toEqual([source.id, 'legacy-source'].sort());
    expect([...cancelledTaskIds(metadata)]).toEqual([copyId]);
    expect(carryCandidates([source, independent], TODAY, cancelledSources(metadata))).toEqual([independent]);
  });

  it('maps a rule skip to its exact occurrence without suppressing other days or rules', () => {
    const recurrence = rule();
    const metadata = {[SKIP_PREFIX + recurrence.id]: TODAY, [SKIP_PREFIX + 'bad']: 'not-a-day', [SKIP_PREFIX + 'empty']: null};
...
describe('carry lineage and replay identity', () => {
  it('suppresses every ancestor when a completed descendant exists, retaining independent homonyms', () => {
    ...
    const child = carryRow(root, USER_ID, new Date('2026-09-03T10:00:00+08:00'));
    const grandchild = {...carryRow(child, USER_ID, new Date('2026-09-06T10:00:00+08:00')), completed: true};
    const independent = task({id: 'unrelated-root', name: root.name, created_at: root.created_at});
    const current = task({id: 'current-day'});
    const future = task({id: 'future-day', created_at: '2026-09-09T01:00:00Z'});
    const rows = [grandchild, independent, future, child, current, root];
    expect(carryCandidates(rows, TODAY).map(t => t.id)).toEqual([independent.id]);
    expect(carryCandidates([...rows].reverse(), TODAY).map(t => t.id)).toEqual([independent.id]);
    expect(carryCandidates([root, child, independent], TODAY).map(t => t.id).sort()).toEqual([child.id, independent.id].sort());
  });

  it('uses stable namespaced UUIDs, distinguishing source, day, and operation', () => {
    const ids = [carryId('source-a', TODAY), carryId('source-b', TODAY), carryId('source-a', '2026-09-09'), repeatId('source-a', TODAY)];
    expect(new Set(ids).size).toBe(4);
    for (const id of ids) expect(id).toMatch(/^[\da-f]{8}-[\da-f]{4}-5[\da-f]{3}-[89ab][\da-f]{3}-[\da-f]{12}$/);
  ...
  it.each([null, '2026-09-08T01:59:59Z', '2026-09-08T02:00:00Z', 'invalid'])('resets elapsed/completion and clears expired or invalid reminder %s without changing history', reminder_at => {
    const source = task({created_at: '2026-09-07T01:00:00Z', elapsed_seconds: 7200, completed_at: '2026-09-07T03:00:00Z', reminder_at, reminder_method: 'sound'});
    const before = structuredClone(source);
    const copy = carryRow(source, USER_ID, new Date(NOW));
    expect(copy).toMatchObject({id: carryId(source.id, TODAY), name: source.name, priority: source.priority, elapsed_seconds: 0, completed: false, completed_at: null, reminder_at: null, reminder_method: null});
    expect(dayKey(copy.created_at)).toBe(TODAY);
    expect(source).toEqual(before);
  });

  it('retains a future reminder and its delivery method', () => {
    const source = task({reminder_at: '2026-09-09T03:00:00Z', reminder_method: 'sound'});
    expect(carryRow(source, USER_ID, new Date(NOW))).toMatchObject({reminder_at: source.reminder_at, reminder_method: 'sound'});
  });
});

describe('recurrence calendar and occurrence identity', () => {
  it.each([
    ['daily', [], ['2026-09-07', '2026-09-08', '2026-09-09', '2026-09-10', '2026-09-11', '2026-09-12', '2026-09-13']],
    ['weekdays', [], ['2026-09-07', '2026-09-08', '2026-09-09', '2026-09-10', '2026-09-11']],
    ['weekly', [0, 2, 6], ['2026-09-08', '2026-09-12', '2026-09-13']],
  ] as const)('%s selects the correct dates in a full week', (frequency, weekdays, expected) => {
    const recurrence = rule({frequency, weekdays: [...weekdays]});
    const week = Array.from({length: 7}, (_, i) => `2026-09-${String(i + 7).padStart(2, '0')}`);
    expect(week.filter(day => isRuleDue(recurrence, day))).toEqual(expected);
  });
  ...
  it('creates only the requested Shanghai day with a +08 reminder and stable ID across reload times', () => {
    const recurrence = rule({startDate: '2020-01-01', reminderTime: '09:15', reminderMethod: 'sound'});
    const first = repeatRow(recurrence, USER_ID, new Date(NOW));
    const replay = repeatRow(recurrence, USER_ID, new Date('2026-09-08T23:59:00+08:00'));
    expect(first).toMatchObject({id: repeatId(recurrence.id, TODAY), elapsed_seconds: 0, completed: false, reminder_at: '2026-09-08T01:15:00.000Z', reminder_method: 'sound'});
    expect(dayKey(first.created_at)).toBe(TODAY);
    expect(replay.id).toBe(first.id);
    expect(repeatRow(recurrence, USER_ID, new Date('2026-09-09T00:01:00+08:00')).id).not.toBe(first.id);
    expect(repeatRow(rule(), USER_ID, new Date(NOW))).toMatchObject({reminder_at: null, reminder_method: null});
  });

  it('ignores malformed or unrelated metadata without losing valid settings', () => {
    const valid = rule();
    expect(rulesFromMetadata({[RULE_PREFIX + valid.id]: valid, arbitrary: valid, [RULE_PREFIX + 'bad']: {...valid, reminderTime: '25:90'}, [RULE_PREFIX + 'null']: null})).toEqual([valid]);
  });
});
...
describe('CSV fidelity and spreadsheet formula safety', () => {
  it('round-trips Unicode, commas, quotes and embedded line breaks with a BOM', () => {
    ...
    expect(records[1]).toEqual([name, '中', '未完成', TODAY, '', '3671', '', task().id]);
  });

  it.each(['=HYPERLINK("https://invalid.example")', '+SUM(1,2)', '-1+2', '@SUM(A1)', '  =1+1', '\t=1+1', '\r=1+1'])('neutralizes formula-like cell %j', name => {
...
describe('reminder validation and catch-up', () => {
  it('rejects invalid timestamps and normalizes absent or default reminder methods', () => {
    const input = {name: '  保留任务  ', priority: 'medium' as const, reminder_at: null, reminder_method: 'sound' as const};
    expect(validInput(input)).toMatchObject({name: '保留任务', reminder_method: null});
    expect(() => validInput({...input, reminder_at: 'not-a-date'})).toThrow('提醒时间无效');
    expect(validInput({...input, reminder_at: NOW, reminder_method: null}).reminder_method).toBe('dialog');
  });
  ...
  it('catches overdue and exactly-due reminders chronologically, excluding invalid, future, completed and dismissed occurrences', () => {
    const rows = [
      ...
      task({id: 'done', completed: true, reminder_at: '2026-09-08T01:00:00Z'}),
      task({id: 'dismissed', reminder_at: '2026-09-08T01:00:00Z'}),
      task({id: 'none'}),
    ];
    const seen = new Set([reminderKey(rows[5])]);
    expect(dueReminders(rows, seen, Date.parse(NOW)).map(t => t.id)).toEqual(['old', 'due']);
    const rescheduled = {...rows[5], reminder_at: '2026-09-08T01:30:00Z'};
    expect(dueReminders([rescheduled], seen, Date.parse(NOW))).toEqual([rescheduled]);
  });
});

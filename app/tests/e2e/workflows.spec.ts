...
test("today tasks default to priority order and keyboard dragging persists a custom order", async ({
  page,
  backend,
}) => {
  backend.tasks.push(
  ...
  await openApp(page);
  const names = () =>
    page.getByTestId("task-card").locator("h3").allTextContents();
  ...
  const handle = page.getByRole("button", { name: "拖动排序：最高优先任务" });
  await handle.focus();
  ...
  await handle.press("Space");
  await expect
    .poll(async () => (await names()).slice(0, 3))
    .toEqual(["今日 Alpha 计划", "最高优先任务", "最低优先任务"]);
  await expect(page.locator(".sync-status")).toHaveText("已同步");
  expect(backend.tasks.find((t) => t.name === "最高优先任务")?.sort_order).toBe(
    2000,
  );
...
test("quick add trims a task, persists priority and survives reload", async ({
  page,
  backend,
}, testInfo) => {
  await openApp(page);
  ...
  await page
    ...
    .fill("  新的高优先级任务  ");
  await page
    .getByRole("combobox", { name: "快速任务优先级" })
    .selectOption("high");
...
test("failed form save retains the full draft and retry writes it once", async ({
  page,
  backend,
}) => {
  await openApp(page);
  ...
  await dialog
    .getByLabel("提醒时间（可选）", { exact: true })
    .fill("2026-09-09T14:35");
  ...
  await expect(
    dialog.getByRole("textbox", { name: "任务名称", exact: true }),
  ).toHaveValue("保留失败草稿");
  await expect(dialog.getByLabel("优先级", { exact: true })).toHaveValue("low");
  await expect(
    dialog.getByLabel("提醒时间（可选）", { exact: true }),
  ).toHaveValue("2026-09-09T14:35");
  await expect(dialog.getByLabel("提醒方式", { exact: true })).toHaveValue(
    "sound",
  );
  ...
  expect(backend.tasks.filter((t) => t.name === "保留失败草稿")).toEqual([
    expect.objectContaining({
      priority: "low",
      reminder_at: "2026-09-09T06:35:00.000Z",
      reminder_method: "sound",
    }),
  ]);
});

test("selected carry preserves history, resets elapsed, and a stale tab replay cannot overwrite the child", async ({
  page,
  context,
  backend,
}) => {
  const prior = structuredClone(backend.tasks[2]),
    older = structuredClone(backend.tasks[3]);
  await openApp(page);
  ...
  for (const tab of [page, stale]) {
    ...
    await picker
      .getByRole("checkbox", { name: new RegExp(prior.name) })
      .check();
    await expect(
      picker.getByRole("checkbox", { name: new RegExp(older.name) }),
    ).not.toBeChecked();
  }
  await page
    .getByRole("button", { name: "迁移 1 项到今天", exact: true })
    .click();
  await expect(page.getByRole("dialog")).toHaveCount(0);
  const childId = carryId(prior.id, TODAY);
  expect(backend.tasks.find((t) => t.id === childId)).toMatchObject({
    name: prior.name,
  ...
  await card(page, prior.name)
    .getByRole("button", { name: `完成：${prior.name}`, exact: true })
    .click();
  await expect(
    card(page, prior.name).getByRole("button", {
      name: `标记未完成：${prior.name}`,
      exact: true,
    }),
  ).toHaveAttribute("aria-pressed", "true");
  const childBeforeReplay = structuredClone(
    backend.tasks.find((t) => t.id === childId),
  );
  await stale
    .getByRole("button", { name: "迁移 1 项到今天", exact: true })
    .click();
  ...
  expect(backend.tasks.filter((t) => t.id === childId)).toEqual([
    childBeforeReplay,
  ]);
  expect(backend.tasks.find((t) => t.id === prior.id)).toEqual(prior);
  expect(backend.tasks.find((t) => t.id === older.id)).toEqual(older);
  expect(backend.tasks.some((t) => t.id === carryId(older.id, TODAY))).toBe(
    false,
  );
...
test("missed reminders catch up on load and dismissal survives reload without completing the task", async ({
  page,
  backend,
}) => {
  const overdue = task({
  ...
  backend.tasks = [
    ...
    task({
      id: "future-reminder",
      name: "稍后的提醒",
      reminder_at: "2026-09-08T04:00:00Z",
      reminder_method: "dialog",
    }),
  ];
  await openApp(page);
...
test("search, date and completion filters compose and clear without leaking today into history", async ({
  page,
}) => {
  await openApp(page);
  await page
  ...
  await page
    .getByRole("combobox", { name: "完成状态筛选" })
    .selectOption("done");
...
test("delete can be undone, remains pending through reload, and only commits after eight seconds", async ({
  page,
  backend,
}) => {
  ...
  await card(page, original.name)
    .getByRole("button", { name: `删除：${original.name}`, exact: true })
    .click();
...
test("a saved repeat rule persists metadata, creates only today once, and does not overwrite completed occurrences on reload", async ({
  page,
  backend,
}) => {
  await openApp(page);
  ...
  await panel
    .getByRole("textbox", { name: "重复任务名称", exact: true })
    .fill("每天站会");
  ...
  await expect(panel.getByRole("list", { name: "重复规则" })).toContainText(
    "每天站会",
  );
  await panel.getByRole("button", { name: "关闭面板", exact: true }).click();
  await expect(card(page, "每天站会")).toBeVisible();
  const entry = Object.entries(backend.metadata).find(([key]) =>
    key.startsWith(RULE_PREFIX),
  );
  ...
  expect(backend.tasks.filter((t) => t.name === "每天站会")).toEqual([
    expect.objectContaining({ id: generatedId }),
  ]);
  await card(page, "每天站会")
    .getByRole("button", { name: "完成：每天站会", exact: true })
    .click();
  await expect(
    card(page, "每天站会").getByRole("button", {
      name: "标记未完成：每天站会",
      exact: true,
    }),
  ).toBeVisible();
  const completed = structuredClone(
    backend.tasks.find((t) => t.id === generatedId),
  );
  for (let reload = 0; reload < 2; reload++) {
    ...
    await expect(card(page, "每天站会")).toBeVisible();
    await expect
      .poll(() => backend.taskRequests("POST").length)
...
test("deleting a repeated occurrence persists its skip marker so reload cannot recreate it", async ({
  page,
  backend,
}) => {
  ...
  await card(page, recurrence.name)
    .getByRole("button", { name: `删除：${recurrence.name}`, exact: true })
    .click();
  ...
  expect(
    backend.tasks.some((t) => t.id === repeatId(recurrence.id, TODAY)),
  ).toBe(false);
});

test("mobile tasks precede an idle timer and a selected timer stays fixed without overflow", async ({
  page,
  backend,
}, testInfo) => {
  ...
  const layout = await page.evaluate(() => {
    const taskArea = document
      .querySelector(".task-area")!
      .getBoundingClientRect();
    const timer = document
      .querySelector(".timer-panel")!
      .getBoundingClientRect();
    return {
  ...
  await expect(
    page.getByRole("combobox", { name: "计时方式", exact: true }),
  ).toBeVisible();
  await card(page, "今日 Alpha 计划")
    .getByRole("button", { name: "计时：今日 Alpha 计划", exact: true })
    .click();
  const selectedTimer = page.locator(".timer-panel");
  await expect(selectedTimer).toHaveCSS("position", "fixed");
  await expect(
    selectedTimer.getByRole("button", { name: "暂停", exact: true }),
  ).toBeVisible();
  ...
  expect(selectedLayout.right).toBeLessThanOrEqual(
    selectedLayout.viewportWidth,
  );
  expect(selectedLayout.bottom).toBeLessThanOrEqual(
    selectedLayout.viewportHeight,
  );
  expect(
    selectedLayout.viewportHeight - selectedLayout.bottom,
  ).toBeLessThanOrEqual(32);
  expect(selectedLayout.scrollWidth).toBeLessThanOrEqual(
    selectedLayout.viewportWidth + 1,
  );
  await page.screenshot({
    path: testInfo.outputPath("mobile-selected-timer.png"),
  });
  await toggle.click();
  await expect(page.locator(".timer-body")).toBeHidden();
  await expect(selectedTimer).toHaveCSS("position", "fixed");
...
test("JSON/CSV downloads include all tasks and JSON settings, excluding session credentials", async ({
  page,
  backend,
}) => {
  ...
  async function download(label: string) {
    ...
    expect(contents).not.toMatch(
      /access_token|refresh_token|sb-backend-auth-token|test-only-not-a-real-refresh-token/,
    );
  ...
  expect(backup.settings).toEqual({
    dtl_carry_prompt_v2: TODAY,
    [RULE_PREFIX + recurrence.id]: recurrence,
  });
  const csv = await download("导出 CSV 表格");
  expect(csv.name).toBe(`每日任务清单-${TODAY}.csv`);
  expect(csv.contents.startsWith("\ufeff")).toBe(true);
  for (const row of backend.tasks) {
    expect(csv.contents).toContain(row.name);
    expect(csv.contents).toContain(row.id);
  }
});

test("crossing midnight updates today, offers yesterday carry, and creates the next repeat occurrence once", async ({
  page,
  backend,
}) => {
  const recurrence = rule();
  ...
  await openApp(page, { keepClock: true });
  await expect(card(page, recurrence.name)).toBeVisible();
  const yesterdayId = repeatId(recurrence.id, TODAY),
    nextId = repeatId(recurrence.id, "2026-09-09");
  await page.clock.runFor(31_000);
  const picker = page.getByRole("dialog", { name: "把未完成的事，接到今天" });
  await expect(picker).toBeVisible();
  await picker
    .getByRole("button", { name: "今天不再提示", exact: true })
    .click();
  await expect(picker).toHaveCount(0);
  await expect
    .poll(() => backend.tasks.some((t) => t.id === nextId))
    .toBe(true);
  await expect(page.getByTestId("task-card")).toHaveCount(1);
  await expect(card(page, recurrence.name)).toBeVisible();
  await expect(card(page, "今日 Alpha 计划")).toHaveCount(0);
...
test("task pagination retains all history beyond a 500-row backend page", async ({
  page,
  backend,
}) => {
  backend.tasks = Array.from({ length: 501 }, (_, i) =>
    task({
  ...
  expect(
    backend.taskRequests("GET").some((r) => {
